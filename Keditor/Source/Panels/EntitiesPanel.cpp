...
namespace krakoa::scene::panels
{
	...
	{
		LogEditorInfo( "Entities panel connected" );
	}
		...
		scene.ForEach( [&]( const ecs::Entity& entity )
		{
			...
			ui::DrawTreeNode( tag.GetTag(), eid, flags, [&]( bool opened )
			{
				ui::IsItemClicked( input::MOUSE_LEFT,
					[&]()
					{
						LogEditorDebug( klib::ToString( "[Entity Panel] Selected entity [\"{0}\", {1}]", tag.GetRawTag(), eid ) );
						selectedEntity = eid;
					} );
				...
				ui::popups::DrawMousePopupMenuItem( nullptr, input::MOUSE_RIGHT,
					[&]()
					{
						ui::popups::DrawMousePopupMenuItem( "Delete Entity",
							[&]()
							{
								LogEditorDebug( klib::ToString( "[Entity Panel] Deleting entity [\"{0}\", {1}]", tag.GetRawTag(), eid ) );
