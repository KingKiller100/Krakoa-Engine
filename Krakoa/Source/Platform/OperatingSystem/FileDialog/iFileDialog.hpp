...
namespace krakoa::os
{
	...
	public:
		...
		USE_RESULT virtual std::filesystem::path OpenFile(const std::wstring_view filter) const = 0;
		USE_RESULT virtual std::filesystem::path SaveFile(const FileDialogFilter& filter) const = 0;
		USE_RESULT virtual std::filesystem::path SaveFile(const std::wstring_view filter) const = 0;
	};
}
