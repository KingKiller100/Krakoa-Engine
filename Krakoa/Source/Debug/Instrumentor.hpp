...
#	define KRK_PROFILE_SCOPE(name) klib::kProfiler::Profiler<std::time_t> timer##__LINE__(name, [&](const klib::kProfiler::ProfilerResult<std::time_t>& result) { krakoa::Instrumentor::Get().WriteProfile(result); })
